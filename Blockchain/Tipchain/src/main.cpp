    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
        ...
        if (fAddressIndex) {
            ...
            for (unsigned int k = tx.vout.size(); k-- > 0;) {
                ...
                if(out.scriptPubKey.IsNeedSpecialTreat(vch))
                {
                    CScript temp = out.scriptPubKey;
                    temp.erase(temp.begin(),temp.begin()+1+2+vch.size());
                    if(temp.IsPayToScriptHash())
                    {
                        vector<unsigned char> hashBytes(temp.begin()+2, temp.begin()+22);
                    ...
                    else if(temp.IsPayToPublicKeyHash())
                    {
                        vector<unsigned char> hashBytes(temp.begin()+3, temp.begin()+23);
                    ...
                    else if(temp.IsPayToPublicKey())
                    {
                        uint160 uhashBytes = Hash160(temp.begin()+1, temp.begin()+34);
        ...
        if (i > 0) { // not coinbases
            ...
            for (unsigned int j = tx.vin.size(); j-- > 0;) {
                ...
                if (fAddressIndex) {
                    ...
                    //add 20180612 begin
                    vector<unsigned char> vch;
                    if(prevout.scriptPubKey.IsNeedSpecialTreat(vch))
                    {
                        CScript temp = prevout.scriptPubKey;
                        temp.erase(temp.begin(),temp.begin()+1+2+vch.size());
                        if(temp.IsPayToScriptHash())
                        {
                            vector<unsigned char> hashBytes(temp.begin()+2, temp.begin()+22);
					...
					for (size_t j = 0; j < tx.vin.size(); j++) {
						...
						int addressType;
                            ...
                            if(prevout.scriptPubKey.IsNeedSpecialTreat(vch))
                            {
                                 ...
                                 if(temp.IsPayToScriptHash())
                                 {
                                     hashBytes = uint160(vector<unsigned char>(temp.begin()+2,temp.begin()+22));
                                     addressType = 2;
                                 }
                                 else if(temp.IsPayToPublicKeyHash())
                                 {
                                     hashBytes = uint160(vector<unsigned char>(temp.begin()+3,temp.begin()+23));
                                     addressType = 1;
                                 }
                                 else if(temp.IsPayToPublicKey())
                                 {
                                     hashBytes = Hash160(temp.begin()+1,temp.begin()+34);
                                     addressType = 1;
                                 }
                                 else{
                                     hashBytes.SetNull();
...
bool InitBlockIndex(const CChainParams& chainparams) 
{
    ...
    //modi by 20180612 for effective only first time
